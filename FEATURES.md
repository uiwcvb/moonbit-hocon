...
- 全部容器共享身份、渲染的默认来源/用户注释与环境来源遮罩、完整 ConfigValueFactory/JVM 类型集成；已实现 ConfigValue/Object/List、常用不可变容器操作、分阶段解析和持久化 Config。Config.entrySet 仍使用既有普通叶值数据接口。
...
- 独立大配置性能、内存/吞吐、多平台及持续负载。五进程计时中，普通浮点列表读取、长极小数渲染相对 0.17 耗时减少 21.3%、96.2%，当前/原版耗时比为 20.350、18.625；长中点渲染与长引号数字读取当前/原版仍为 501.845、7.978。旧 parse-64 相对 0.17 为 1.026。最大进程中位数波动比 2.466，不稳定标记 true。完整性能仍未追平。 受控 GC 不代表生产内存验证。
