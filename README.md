> 2026-09-22 当前本地版 0.23.0：申报定位为“HOCON 配置迁移与启动校验”。已更新[现有项目对照](DUPLICATION.md)、[申报草稿](PROPOSAL.md)及[本轮验证](evidence/innovation-review-20260922/results.json)。下面带日期的旧轮次描述保留历史范围；团队已有公开仓库，本次本地修订尚未由本任务推送。
...
ConfigValue 和 ConfigEntry 使用原生私有字段保存内部状态，公开对象仍冻结，反射不暴露内部字段，伪造原型或代理不能绕过类型检查。条目枚举直接创建保留树的私有视图，省去中间 Map 和叶值深复制；每次仍返回新的可修改集合。公开 MoonBit `entry_set` 保持深复制的可变数据契约，私有视图仍执行原有深度/展开限额验证；解包、派生和 Worker 边界的隔离不变。

32 位数字哈希直接扫描 ASCII 整数，保留可选正负号、前导零、最小负整数及溢出后回退。独立 JDK Integer.valueOf 的 2,342 个不同输入进入 19 个双后端分组；该矩阵只验证不超过 11 个 UTF-16 单元的快路径范围，不是完整数字解析规范。路径生成直接扫描 UTF-16，ASCII 快速判断；BMP 仍采用固定 JDK 的字符表，代理对仍需引用。现有 4,380 个树操作/路径枚举原版程序涵盖字符表边界。

集合插入只计算一次条目哈希；集合整体哈希直接遍历私有存储。没有缓存枚举集合或枚举结果。新增三项宿主检查覆盖私有字段品牌、共享私有叶值隔离，以及与核心枚举一致的路径/深度/延迟对象错误；全部条目宿主检查为 20 项。新增受控 GC 检查分别释放 Config、集合、条目和值包装，保留一个可读叶值对照；不作为完整内存追平的证明。

`benchmark-entry-scan.mjs` 将 12 个类型化枚举负载与固定 0.19 及原版比较，全部运行相同公开行为。另用 `benchmark-entry-scan-regression.mjs` 对照既有 29 个解析/读取/编辑/渲染负载。完整追平仍未完成。
...
长极小数在 32 位前缀区间仍不确定时，保留 768 位有效数字，并利用非零尾数确定精确中点方向，避免回退到完整长系数转换。全部输入仍需线性扫描完成语法验证；大整数转换只需处理有界前缀，普通数量级保留原路径。

证明：正数的每个有限 binary64 舍入中点都是 `2^-1075` 的整数倍。十进制数量级 `m <= -308`、保留 768 位时，前缀格距为 `10^(m-767)`，指数不大于 `-1075`。所有二进制中点都落在这条十进制格线上，连续格点间不存在舍入中点。截断后的原值严格位于两格点之间，因此只有下界本身恰为中点时需要向上舍入。有效位数已经排除了尾零；仅当确实截去非零尾数时才启用该规则，精确中点仍采用偶数舍入。符号由既有外层转换处理，包含负零。前缀尾零压缩不改变下界精确值或这项证明。

独立 JDK 解析矩阵增加 1,770 个不同输入，覆盖 767/768/769 位边界、超过 4,000 位尾数、两种中点奇偶、次正规/正规交界及 `1e-307` 路径边界，共 29,621 个字符串、232 个双后端分组。期望来自 JDK Double.parseDouble 的位结果，未使用本实现计算期望。

`tools/benchmark-decimal-bound.mjs` 将 31 个负载与固定 0.20 和原版比较，包含三个长中点渲染负载以及已有解析/读取/编辑/比较负载。最终结果与封存见 `evidence/decimal-bound-performance.json` 和 `evidence/decimal-bound-upgrade.json`；完整追平目标仍未完成。

## 0.21 最终验证

JS/Wasm-GC 各 16,685 项通过；独立 JDK 解析矩阵共 29,621 个字符串（新增 1,770）、232 个分组。数值读取/渲染实时原版对照共 5,702 次；15 个保存原版套件回放共 31,776 次，含历史重叠，不计为全部新独立案例。216 个源码/生成文件再生一致，交付引擎与编译结果相同；既有宿主、异步、回收和资源限额检查全部通过。

五进程 31 个负载与固定 0.20、原版对照，结果全部一致。下表是进程中位耗时之比，小于 1 表示当前实现更快。

| 负载 | 当前/0.20 | 当前/原版 |
| --- | ---: | ---: |
| render-json-midpoint-long | 0.164 | 81.460 |
| render-json-midpoint-normal | 0.093 | 66.064 |
| render-json-midpoint-upper-tiny | 0.097 | 68.184 |
| retained-double-long-text-list | 0.150 | 1.401 |
| retained-double-ordinary-list | 0.974 | 20.302 |

最大当前/0.20 耗时比为 1.104（legacy-fallback-32）；最大进程中位数波动比 2.610，不稳定标记 true。此处只测已解析配置的热调用及既有两个完整解析请求；没有证明启动、峰值内存、长期运行、全部输入或跨平台性能。完整功能、兼容性及性能仍未追平。

## 0.22 列表直接转换与普通小数扫描

类型化列表逐项调用值转换函数，去掉为每个元素创建临时配置对象和解析固定路径的开销。保留空值/未解析元素诊断、原始 Bare 转换、配置列表的已知未解析对象行为，以及返回数组和对象的复制隔离。

十进制语法扫描同时累积至多 16 位有效系数。只有系数严格小于 `2^53` 且十进制指数在 `[-22,22]` 时，才使用一次浮点乘除完成转换：系数和这些十进制幂均可精确表示，单次操作给出正确舍入。严格界限排除累积阶段已经舍入到 `2^53` 的整数；更多位数或指数继续使用原有精确路径。优化使用可移植 MoonBit 代码，不缓存输入或查询结果。

JDK 解析矩阵新增 1,516 个不同字符串，覆盖 `2^53` 两侧、15/16/17 位系数、`10^22` 两侧、符号和点位置，现为 31,137 个字符串、244 个双后端分组。另有 3 个核心测试检查元素诊断、原始标量转换、返回值修改隔离和未解析对象。

`profile-double-read.mjs` 对普通浮点列表读取采样，原始 CPU profile 仅用于定位分配和重复转换，不作为计时结果。`benchmark-list-read.mjs` 对 34 个公开 API 负载做五进程当前/固定 0.21/原版比较；包含已有负载、64/512 项混合数值列表和 16 项精确操作列表。最终证据见 `list-read-upgrade.json`，完整追平尚未完成。

## 0.22 最终验证

JS/Wasm-GC 各 16,700 项通过；独立 JDK 解析矩阵共 31,137 个字符串（新增 1,516）、244 个分组，另有 3 个列表诊断/复制隔离/未解析对象测试。集合/数值/时间/渲染实时原版对照共 9,173 次；15 个保存原版套件回放共 31,776 次，含历史重叠，不计为全部新独立案例。221 个源码/生成文件再生一致，交付引擎与编译结果相同；既有宿主、异步、回收和资源限额检查全部通过。

五进程 34 个负载与固定 0.21、原版对照，结果全部一致。下表是进程中位耗时之比，小于 1 表示当前实现更快。

| 负载 | 当前/0.21 | 当前/原版 |
| --- | ---: | ---: |
| retained-double-ordinary-list | 0.620 | 12.478 |
| retained-double-exact-16 | 0.339 | 2.790 |
| retained-double-mixed-64 | 0.572 | 7.481 |
| retained-double-mixed-512 | 0.407 | 7.142 |
| retained-double-tiny-list | 0.870 | 3.431 |
| retained-double-long-text-list | 1.009 | 1.197 |
