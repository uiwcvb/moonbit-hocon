...
for(const mode of ['value-read','value-hash','value-equals','value-search','value-fallback'])cases.push({name:mode+'-32',mode,source:valueSource,repeats:32});
if(process.argv[2]==='--worker'){
 const {inspect_json}=await import(pathToFileURL(process.argv[3]));
 const {Config}=await import(pathToFileURL(process.argv[4]));
 for(const item of cases){
  if(item.mode?.startsWith('value-')&&!Config.prototype.getValue)continue;
  let work;
  if(item.mode?.startsWith('value-')){
 ...
 const baselineCommit='c34c00d1b43f9c4acb16803922823bc2cb6fb680';
 ...
 const folder=fs.mkdtempSync(path.join(os.tmpdir(),'hocon-native-read-perf-')),baselineFile=path.join(folder,'web/engine.mjs');
  ...
  for(let campaign=0;campaign<5;campaign++){
   for(const variant of campaign%2?['current','baseline','upstream']:['upstream','baseline','current']){
    if(variant==='upstream'){
     ...
     const values=run(process.env.JAVA??'java',['-cp',jar,fileURLToPath(new URL('./ValueOracle.java',import.meta.url)),'--benchmark'],cases.slice(10).map(JSON.stringify).join('\n')+'\n');
     records.upstream.push([...legacy,...retained,...values].map((row,i)=>({...row,name:cases[i].name})));
   ...
   console.log('Completed value/container performance campaign '+(campaign+1)+'/5');
  }
