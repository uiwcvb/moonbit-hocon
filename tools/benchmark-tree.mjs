...
const inputs=[
 ...
 {name:'set-paths-64',newGetter:true,request:{source:nested,operations:[0,1,2,3].map(i=>({op:'with-value',path:'k'+i+'.port',value:8080}))}},
 {name:'remove-paths-64',newGetter:true,request:{source:nested,operations:[0,1,2,3].map(i=>({op:'without-path',path:'k'+i+'.port'}))}},
 {name:'select-subtree-64',newGetter:true,request:{source:nested,operations:[{op:'with-only-path',path:'k32'}]}},
 {name:'entries-64',newGetter:true,request:{source:nested,getter:'entries'}},
 {name:'validate-64',newGetter:true,request:{source:nested,checkValid:{source:nested}}},
];
if(process.argv[2]==='--worker'){
 ...
 const baseline=spawnSync('git',['show','6279e63afb541f5152c7827aa031f7be37fa914c:web/engine.mjs'],{cwd:root,maxBuffer:16*1024*1024});assert.equal(baseline.status,0,String(baseline.stderr));
 ...
 try{
  ...
  const report={unstableMeasurements,performanceParityEstablished:false,utc:new Date().toISOString(),cpu:os.cpus()[0]?.model,node:process.version,java:records.upstream[0][0].java,processesPerVariant:5,warmupPerProcess:30,samplesPerProcess:15,executionsPerSample:3,baselineCommit:'6279e63afb541f5152c7827aa031f7be37fa914c',baselineEngineSha256:hash(baseline.stdout),engineSha256:hash(fs.readFileSync(current)),jarSha256:hash(fs.readFileSync(jar)),scope:'Warm end-to-end request JSON parse, HOCON parse/resolve/tree edits/validation/getter, and JSON response rendering in Node/MoonBit and unmodified Lightbend Config. No startup, file I/O, peak memory or steady-load claim. Fixed alternating process order; equality checked for every process result.',summaries,records};
  fs.writeFileSync(new URL('../evidence/tree-performance.json',import.meta.url),JSON.stringify(report,null,2)+'\n');
