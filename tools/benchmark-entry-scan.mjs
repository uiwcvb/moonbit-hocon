...
if(process.argv[2]==='--worker'){
 ...
 const jar=process.env.HOCON_REFERENCE_JAR;assert(jar);const baselineCommit='926655aa15f60e5fb321d557cbe07d65f6489702',folder=fs.mkdtempSync(path.join(os.tmpdir(),'hocon-entry-scan-perf-'));
 ...
 try{
  ...
  const report={utc:new Date().toISOString(),baselineCommit,baselineEngineSha256:sha(fs.readFileSync(path.join(folder,'web/engine.mjs'))),engineSha256:sha(fs.readFileSync(path.join(root,'web/engine.mjs'))),jarSha256:sha(fs.readFileSync(jar)),java:records.upstream[0][0].java,node:process.version,processesPerWorkload:5,warmupCalls:30,samplesPerProcess:15,callsPerSample:3,scope:'Twelve typed retained-config enumeration/count and enumeration/hash workloads compare current, fixed 0.19 and native Config. Sixteen enumerations per call; construction and output validation outside timing. All variants exercise the same public behavior. Each call constructs a fresh detached mutable set; no set/result cache is used',unstableMeasurements:summaries.some(s=>['current','baseline','upstream'].some(v=>s[v]?.processSpread>1.5)),performanceParityEstablished:false,sourceHashes:sourceHashes(),summaries,records};
  fs.writeFileSync(path.join(root,'evidence/entry-scan-performance.json'),JSON.stringify(report,null,2)+'\n');for(const s of summaries)console.log(s.name+' current/native='+s.currentOverUpstream.toFixed(3)+(s.baseline?' current/0.19='+s.currentOverBaseline.toFixed(3):''));
