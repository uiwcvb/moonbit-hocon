...
if(process.argv[2]==='--worker'){
 ...
 const baselineCommit='80363743d7bda9391cefcad6fba6b7cc15a45017';
 ...
 const folder=fs.mkdtempSync(path.join(os.tmpdir(),'hocon-double-render-perf-')),baselineFile=path.join(folder,'web/engine.mjs');
  ...
  for(let campaign=0;campaign<5;campaign++){
   for(const variant of campaign%2?['current','baseline','upstream']:['upstream','baseline','current']){
    if(variant==='upstream'){
     ...
     const renders=run(process.env.JAVA??'java',['-cp',jar,fileURLToPath(new URL('./RenderOracle.java',import.meta.url)),'--benchmark'],cases.slice(16).map(JSON.stringify).join('\n')+'\n');
     records.upstream.push([...legacy,...retained,...values,...renders].map((row,i)=>({...row,name:cases[i].name})));
