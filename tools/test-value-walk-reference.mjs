...
import {requests,execute} from './value-walk-cases.mjs';
const golden=process.argv.includes('--golden');let references,reference;
if(golden){const saved=JSON.parse(fs.readFileSync(new URL('../evidence/value-walk-reference-vectors.json',import.meta.url),'utf8'));assert.deepEqual(saved.requests,requests);({references,reference}=saved);}
else{
 ...
 fs.writeFileSync(new URL('../evidence/value-walk-reference-vectors.json',import.meta.url),JSON.stringify({reference,requests,references},null,2)+'\n');
}
const failures=[];
...
const report={utc:new Date().toISOString(),mode:golden?'saved independent native results':'live official library',reference,total:requests.length,passed:requests.length-failures.length,failed:failures.length,scope:'Direct traversal: numeric width/float boundaries, object key-set prechecks and native short-circuit order, unresolved directional equality, early/late/absent list searches through 512 entries, varied-size object equality/hash. Exact values and acceptance; diagnostic text and tree-bin iteration remain out of scope.',sourceHashes:sourceHashes(),failures};
fs.writeFileSync(new URL('../evidence/value-walk-reference-'+(golden?'replay':'validation')+'.json',import.meta.url),JSON.stringify(report,null,2)+'\n');
console.log(`${report.passed}/${report.total} value traversal native programs agree`);
