...
before = snapshot()
for command in [['node', 'tools/generate-collection-tests.mjs'], [str(moon), 'fmt'],
...
report = dict(passed=True, files=len(after), checks=['independent test generation', 'format', 'API generation', 'JS build and engine copy'], sha256=after)
(root / 'evidence/collection-generation.json').write_text(json.dumps(report, indent=2) + '\n', encoding='utf-8')
