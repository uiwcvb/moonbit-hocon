...
before = snapshot()
for command in [['node', 'tools/generate-collection-tests.mjs'], ['node', 'tools/generate-http-json-tests.mjs'], ['node', 'tools/generate-path-characters.mjs'], ['node', 'tools/generate-tree-tests.mjs'], [str(moon), 'fmt'],
...
report = dict(passed=True, files=len(after), checks=['independent collection, HTTP JSON, JDK path and tree API test generation', 'format', 'API generation', 'JS build and engine copy'], sha256=after)
(root / 'evidence/tree-generation.json').write_text(json.dumps(report, indent=2) + '\n', encoding='utf-8')
