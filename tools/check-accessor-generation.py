...
report = dict(passed=True, files=len(after), checks=['independent collection, HTTP JSON, JDK path, tree, document and extended accessor API test generation', 'format', 'API generation', 'JS build and engine copy'], sha256=after)
(root / 'evidence/accessor-generation.json').write_text(json.dumps(report, indent=2) + '\n', encoding='utf-8')
