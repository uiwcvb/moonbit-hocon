...
for(const size of [64,512])cases.push({name:'retained-double-mixed-'+size,mode:'double-read',source:'values=['+Array.from({length:size},(_,i)=>[String((i+1)/8),'1e23','1e100','3.141592653589793'][i%4]).join(',')+']',repeats:8});
cases.push({name:'retained-double-exact-16',mode:'double-read',source:'values=['+Array.from({length:16},(_,i)=>String(i+1)+'e'+(i-8)).join(',')+']',repeats:8});
for(const [name,items] of [['unicode',['１２３４５','-１２３４５','١٢٣٤٥','१२३४५']],['mixed-digits',['1٢3','123٤5','12３45','-1234٥']],['ascii-text',['12345','-12345','123456789','0']]])cases.push({name:'retained-double-'+name+'-32',mode:'double-read',source:'values='+JSON.stringify(Array.from({length:32},(_,i)=>items[i%items.length])),repeats:8});
if(process.argv[2]==='--worker'){
 ...
 const baselineCommit='93bd088998856555100d387bddbe30b1e9b347fe';
 ...
 const folder=fs.mkdtempSync(path.join(os.tmpdir(),'hocon-double-direct-')),baselineFile=path.join(folder,'web/engine.mjs');
