...
import {run,json as resolvedJson} from '../web/engine.mjs';
const args=process.argv.slice(2);let input, json=false, resolved=false;
try {
 for(let i=0;i<args.length;i++){
  const a=args[i];
  if(a==='--help'){process.stdout.write('Usage: node tools/cli.mjs [--input TEXT | --file PATH] [--json] [--resolved-json]\nWithout --input/--file, reads UTF-8 stdin. Exit: 0 success, 2 invalid input, 1 host error.\n');process.exit(0)}
 ...
 const output=resolved?resolvedJson(input):run(input),ok=!output.startsWith('ERROR:');
