...
try {
 ...
 if(type!==undefined&&!['value','string','boolean','int','long','double','duration','bytes','memory','list','has','has-or-null','null','entries','empty','resolved','string-list','boolean-list','int-list','long-list','double-list','duration-list','bytes-list','memory-list','config','config-list','number','number-list','object','object-list','any-ref','any-ref-list','enum','enum-list','period','temporal','duration-in','duration-list-in','milliseconds','nanoseconds','milliseconds-list','nanoseconds-list'].includes(type))throw new Error('Unknown getter type: '+type);
 if(options.unit!==undefined&&!['duration-in','duration-list-in'].includes(type))throw new Error('--time-unit requires duration-in or duration-list-in');
 if(['duration-in','duration-list-in'].includes(type)&&!['nanoseconds','microseconds','milliseconds','seconds','minutes','hours','days'].includes(options.unit))throw new Error('Choose --time-unit nanoseconds|microseconds|milliseconds|seconds|minutes|hours|days');
